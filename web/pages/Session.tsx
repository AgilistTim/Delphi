...
<html>
<head>
<title>Delphi Report - ${run.question}</title>
<style>
...
<body>
  <div class="report-print-header">
    <h1>${run.question.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</h1>
    <div class="meta">
      <span>Completed: ${run.completed_at ? new Date(run.completed_at).toLocaleString() : 'N/A'}</span>
      ${run.total_tokens ? `<span>Tokens: ${(run.total_tokens / 1000).toFixed(1)}k</span>` : ''}
      ${run.cost_usd ? `<span>Cost: $${Number(run.cost_usd).toFixed(2)}</span>` : ''}
    </div>
  </div>
  ${report?.convergence_analysis ? `
  <div class="report-print-verdict">
    <div class="type">${report.convergence_analysis.consensus_type || ''}</div>
    ${report.convergence_analysis.confidence_score ? `<div class="score">Confidence: ${report.convergence_analysis.confidence_score}/10</div>` : ''}
    ${report.convergence_analysis.consensus_statement ? `<div class="statement">${report.convergence_analysis.consensus_statement}</div>` : ''}
  </div>` : ''}
  <div class="report-print-body">${content}</div>
  <div class="report-print-footer">
    Generated by Delphi Agent &middot; AI-Augmented Expert Deliberation
  </div>
</body>
</html>`);
