...
export interface DecisionFork {
  prompt: string; // "If the oppositional case is correct, what are you choosing to risk by following the consensus?"
  concrete_risks: string[]; // 2-3 risks extracted from oppositional case + counterfactual analysis
}
...
export interface RegimeDescription {
  scarce_resource: string; // What becomes the scarce resource in this world
  winning_organization: string; // What kind of organization wins
  failure_mode: string; // What failure looks like
}

export interface RegimeSplitAnalysis {
  consensus_regime: RegimeDescription; // Regime A - if the dominant conclusion is correct
  oppositional_regime: RegimeDescription; // Regime B - if the contrarian is correct
  closing_statement: string; // "This decision is about which regime you believe you are entering."
}
...
export interface RegimeSignals {
  consensus_signals: string[]; // 2-3 observable signals that Regime A (consensus) is unfolding
  oppositional_signals: string[]; // 2-3 observable signals that Regime B (oppositional) is unfolding
  intro_statement: string; // "If you had to know which regime is emerging within the next year, these are the signals that would matter most."
}

// Cross-Examination Exchange (#1) - Direct expert-to-expert engagement
...
// Evidence Quality Assessment (#3) - Source quality scoring
export type SourceType = 'academic' | 'government' | 'news' | 'industry' | 'blog' | 'unknown';

export interface EvidenceQuality {
  source_type: SourceType;
  recency_score: number; // 0-1, how recent the source is
  domain_authority: number; // 0-1, estimated authority of the domain
...
// Decision Canvas (#4) - Actionable synthesis of the full analysis
...
// Question Decomposition (#5) - Breaking complex questions into sub-questions
export interface SubQuestion {
  question: string;
  rationale: string;
  dependency: string; // which other sub-questions this depends on
}

export interface QuestionDecomposition {
  is_complex: boolean;
  sub_questions: SubQuestion[];
  interaction_notes: string; // how sub-questions interact
}

// Structured Uncertainty (#6) - Richer confidence expression
export interface StructuredUncertainty {
  overall_confidence: number; // 1-10, same as before for backward compat
  confidence_by_claim: { claim: string; confidence: number }[];
  conditional_confidence: { condition: string; confidence_if_true: number; confidence_if_false: number }[];
  key_assumptions: string[];
}

// Prior Analysis Reference (#7) - Historical learning
...
// Signal Tracking (#7) - Tracking regime signals over time
...
// Retrospective (#8) - Calibration feedback loop
...
// Evidence Contrarian (#11) - Topic-specific counterevidence
...
// Narrow agent role (old type — kept for components that only care about the three archetypes).
export type AgentRole = 'expert' | 'contrarian' | 'orchestrator';
...
export interface APIConfig {
  anthropic: {
    apiKey: string;
    modelLight?: string | undefined;
    modelDefault?: string | undefined;
    modelHeavy?: string | undefined;
    promptCaching?: boolean | undefined;
  };
  openai: {
    // OpenAI is retained only for embeddings (convergence similarity).
    apiKey: string;
...
export interface TokenUsage {
  prompt_tokens: number;        // input tokens (uncached new input)
  completion_tokens: number;    // output tokens
  total_tokens: number;
  cache_read_tokens?: number;   // cache hits (charged at 10%)
  cache_write_tokens?: number;  // cache writes (charged at 125%)
  web_search_queries?: number;
  estimated_cost_usd?: number;
  estimated_cost_pence?: number;
}

// Agent / call-site labels for breakdowns. Keep broad — new labels are added
// whenever a new adapter call site is introduced.
...
export interface CostSummary {
  ...
  total_web_search_queries: number;
  estimated_total_cost_usd: number;
  estimated_total_cost_pence: number;
